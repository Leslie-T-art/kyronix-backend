...
public class SelfAssessmentProblemDetailsHandler {

    @ExceptionHandler(SelfAssessmentNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    ApiResponse<Void> handleNotFound(SelfAssessmentNotFoundException exception) {
        return new ApiResponse<>(false, exception.getMessage(), null, java.time.Instant.now(), null);
    }

    @ExceptionHandler(SelfAssessmentValidationException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    ValidationErrorResponse handleValidation(SelfAssessmentValidationException exception) {
